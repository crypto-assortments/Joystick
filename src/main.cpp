...
unsigned int nStakeMaxAge = 30 * 10 * 60 * 24; // 5 days
unsigned int nStakeMaxAge_2 = 30 * 60 * 60 * 24; // 30 days
unsigned int nModifierInterval = 15 * 60; // 15 minutes (time to elapse before new modifier is computed)

int nCoinbaseMaturity = 40; // 40 blocks for coins to mature
...
const string strMessageMagic = "joystick Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 10 ) // premine 1 000 000
    {
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear = COIN_YEAR_REWARD; // 777%
    int64_t nSubsidy = 0;
   ...
   if (nHeight <= 1000)
   {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * 10 * CENT ; // 10 %
   }
...
// joystick: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "joystick", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
/*
block.GetHash() == 0000e60063b43cd1f8ae6e522c019592f15fcf47331723da6b1e47a678d43f74
block.hashMerkleRoot == db9a235b60a6375db3e061abd4ae12d5e933193eeefe98b3b07ccb28f5393d1c
block.nTime = 1508550472
block.nNonce = 311017
*/
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Nissan GT-R driven 200km/h by Sony PlayStation joystick controller";
        CTransaction txNew;
        txNew.nTime = 1508550472;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486159 << CBigNum(9999) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xdb9a235b60a6375db3e061abd4ae12d5e933193eeefe98b3b07ccb28f5393d1c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x2d, 0x2d, 0xd1, 0xd0 };
